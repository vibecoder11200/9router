...
export async function GET() {
  ...
  try {
    return NextResponse.json(await admin.listProxyGroups(ctx.base, ctx.adminKey));
  } catch (error) {
...
export async function POST(request) {
  ...
  try {
    ...
    return NextResponse.json(await admin.addProxyGroup(ctx.base, ctx.adminKey, body));
  } catch (error) {
    return NextResponse.json({ error: error.message, detail: error.detail }, { status: error.status || 500 });
  }
}
